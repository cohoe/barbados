...
class Make:
    ...
    def run(self):
        ...
        # Validate syntax
        try:
            barbados.util.read_yaml_file(filepath, fatal=False)
            print('Syntax OK!')
        except Exception:
    ...
    def _prepare_default_content(filepath, template, raw_drink_name):
        ...
        default_content = barbados.util.read_file(default_content_source)
    ...
    def _validate_args(args):
        if ' ' in args.drink_name:
            barbados.util.die("Drink name '%s' cannot have spaces. Use dashes." % args.drink_name)

        if not os.path.isdir(args.recipe_dir):
            barbados.util.die("Recipe path '%s' is not a directory" % args.recipe_dir)

        if not os.path.isfile(args.template):
            barbados.util.die("Template path '%s' is not a file." % args.template)
